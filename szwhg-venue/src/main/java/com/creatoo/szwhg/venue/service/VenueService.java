...
public class VenueService {
     ...
     * @return
     */
    public String  addComment(String venId,Comment comment){
        Venue venue=venueDao.findOne(venId);
        if(venue==null) throw new BsException("场馆不存在");
        List<Comment> comments=venue.getComments();
        if (comments == null) {
     ...
     */
    public void deleteComment(String venId,String commentid){
        Venue venue=venueDao.findOne(venId);
        Optional.ofNullable(venue).map(t->t.getComments()).orElse(new ArrayList<>())
     ...
     */
    public void auditComment(String venId,String commentid,CommentStatus commentStatus){
        Venue venue=venueDao.findOne(venId);
        Optional.ofNullable(venue).map(t->t.getComments()).orElse(new ArrayList<>())
                .stream().filter(c->c.getId().equals(commentid)).findFirst().ifPresent(comment->{
